...
class MEAD(data.Dataset):
    ...
    def __init__(self, opt, which='source', phase='train'):
        ...
        for actor in self.selected_actors:
            ...
            ## data_actor: 该speaker所以情绪的所有子目录 -> 二维元组:第一维表示数据 第二维表示数据对应的情绪
        ...
        # ## 音频信息
        # for actor in self.selected_actors:
        #     actor_root_audio = os.path.join("MEAD_data_audio", '{}_audio.pkl'.format(actor))
        #     assert os.path.isfile(actor_root_audio), '%s is not a valid file' % actor_root_audio
        #     ## data_actor: 该speaker所以情绪的所有子目录 -> 二维元组:第一维表示数据 第二维表示数据对应的情绪
        #     data_actor_audio = pickle.load(open(actor_root_audio, "rb"))
        #     audio.extend(data_actor_audio)
        # for a in audio:
        #     params_audio, _ = a
        #     seqs_audio = [params_audio[x:x + self.seq_len] for x in
        #                 range(0, params_audio.shape[0], self.hop_len) if
        #                 len(params_audio[x:x + self.seq_len]) == self.seq_len]
        #     self.seqs_audio.extend(seqs_audio)
        # self.seqs_audio = np.stack(self.seqs_audio, axis=0)
        # ###########################################################
        # ###########################################################
        ...
        if self.which == 'reference':
            ...
            # self.seqs_audio = self.seqs_audio[p]
    ...
    def __getitem__(self, index):
        ...
        # sequence_audio = torch.FloatTensor(self.seqs[index])
