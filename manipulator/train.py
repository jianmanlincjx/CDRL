...
from prompt_learning.supervision_model import EmotionSupervision
import pickle
...
def compute_g_loss(nets, opt, x_real, y_org, y_trg, z_trg=None, x_ref=None, DCL=False, label_index=None):
    ...
    if DCL:
        ...
        x_refs_batch = x_ref.reshape(B*T, E)
        target_codedict = {}
        reference_codedict = {}
        ...
        for key_1, value_1 in content_1.items():
            ...
            new_value_1 = value_1.repeat(B*T, *one_shape)
            new_value_2 = value_1.repeat(B*T, *one_shape)
            target_codedict[key_1] = copy.copy(new_value_1) # c1
            reference_codedict[key_1] = copy.copy(new_value_2) # c2
            if key_1 == "exp":
                target_codedict[key_1] = x_fake_batch[:, 1:] # c1 e2
                reference_codedict[key_1] = x_refs_batch[:, 1:] # c2 e2
            elif key_1 == "pose":
                target_codedict[key_1][:, 3] = x_fake_batch[:, 0] # c1 e2
                reference_codedict[key_1][:, 3] = x_refs_batch[:, 0] # c2 e2
        opdict_target, visdict_target = deca.decode(target_codedict)
        opdict_reference, visdict_reference = deca.decode(reference_codedict)
        ## c1 e2
        y = visdict_target['shape_detail_images']

        ## c2 e2
        z = visdict_reference['shape_detail_images']
        # torchvision.utils.save_image(y, "y.png")
        # torchvision.utils.save_image(z, "z.png")
        # exit()
        loss_emotion = DCL_model(z, y, label_index) * 50
    ...
    if DCL:
        loss = loss_adv + opt.lambda_sty * loss_sty \
            + opt.lambda_cyc * loss_cyc - opt.lambda_mouth * loss_mouth + loss_emotion 
    else:
        loss = loss_adv + opt.lambda_sty * loss_sty \
...
if __name__ == '__main__':
    ...
    ########################################
    ## TCCL
    DCL_model = EmotionSupervision().cuda()
    for param in DCL_model.parameters():
        param.requires_grad = False
    ### DECA
    deca_cfg.model.use_tex = True
    deca = DECA(config = deca_cfg, device=device)
    content_1 = pickle.load(open("/data2/JM/code/NED-main/MEAD_data/Z_Training_Aids/000000_000000.pkl", "rb"))
    # content_2 = pickle.load(open("/data2/JM/code/NED-main/MEAD_data/Z_Training_Aids/000000_000049.pkl", "rb"))
    
    # content_1_audio = np.load("/data2/JM/code/NED-main/MEAD_data/Z_Training_Aids/000000_000000.npy")
    # content_2_audio = np.load("/data2/JM/code/NED-main/MEAD_data/Z_Training_Aids/000000_000049.npy")
    ########################################
    ### initialize models
    ...
    for epoch in range(0 if opt.finetune else opt.resume_epoch, opt.niter):
        ...
        for i in range(len(loader_src)):
            ...
            g_loss, g_losses_latent = compute_g_loss(nets, opt, x_real, y_org, y_trg, z_trg=z_trg, DCL=False)
            reset_grad()
            g_loss.backward()
            ...
            g_loss, g_losses_ref  = compute_g_loss(nets, opt, x_real, y_org, y_trg, x_ref=x_ref, DCL=True, label_index=y_trg)
            reset_grad()
            g_loss.backward()
            optims.generator.step()
            # print(g_losses_ref.pc, g_losses_ref.nc, g_losses_ref.pe, g_losses_ref.ne)
