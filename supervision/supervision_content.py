...
class MaskLearning(nn.Module):
    ...
    def forward(self, img_feature, audio_feature):
        ...
        content_attn_weights = F.softmax(torch.bmm(query.permute(0, 2, 1), key) / (C ** 0.5), dim=-1)
        emotion_attn_weights = 1 - content_attn_weights
        # 利用注意力权重加权求和得到最终的输出
        content_information = torch.bmm(value, content_attn_weights.permute(0, 2, 1)).view(B, C, H, W)
        emotion_information = torch.bmm(value, emotion_attn_weights.permute(0, 2, 1)).view(B, C, H, W)
        return content_information, emotion_information
...
class DecoupledContrastiveLearning(nn.Module):
    ...
    def forward(self, source_img, target_img, source_audio):
        x_f = self.img_encoder(source_img) # c1 e1
        y_f = self.img_encoder(target_img) # c1 e2

        B, C, H, W = x_f.shape
         
        x_fc, _ = self.mask_learning(x_f, source_audio) # c1 e1 
        y_fc, _ = self.mask_learning(y_f, source_audio) # c1 e2
        
        positive_content_similarity = torch.clamp(F.cosine_similarity(x_fc.reshape(B, -1), y_fc.reshape(B, -1)), 0, 1) # c1 and c1

        content_loss = F.binary_cross_entropy(positive_content_similarity, torch.ones_like(positive_content_similarity)) 
        
        return content_loss, torch.mean(positive_content_similarity)
