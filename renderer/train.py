...
### initialize dataset
data_loader = CreateDataLoader(opt, is_train=True)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for idx, data in enumerate(dataset):
        ...
        for i in range(0, n_frames_total-t_len+1, n_frames_load):
            ...
            audio_featue_source = Variable(data['audio_feature'][:, i:(i+t_len), ...]).view(-1, t_len, 1, 512)
            ...
            fake_B, real_A, real_Bp, fake_B_last, audio_information = modelG(input_A, input_B, fake_B_last, audio_featue_source)
            if i == 0:
            ...
            losses = modelD(tensor_list, mouth_centers, eyes_centers, audio_information)
            ...
            loss_G = loss_dict['G_GAN'] + loss_dict['G_GAN_Feat'] + loss_dict['G_VGG'] + loss_dict['dcl']*5
