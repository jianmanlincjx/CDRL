...
class videoDataset(BaseDataset):
    def initialize(self, opt, is_train=False):
        self.opt = opt
        ...
        #get audio feature
        if self.is_train:
            self.audio_feature = os.path.join(opt.celeb, "audio_feature")
            self.audio_feature_paths = make_video_dataset(self.audio_feature, opt.max_n_sequences)
            assert_valid_pairs(self.nmfc_video_paths, self.audio_feature_paths)
        ###################
    ...
    def __getitem__(self, index):
        ...
        ####
        if self.is_train:
            audio_feature_paths = self.audio_feature_paths[seq_idx]
        ...
        A_paths = []
        if self.is_train:
            rgb_video = nmfc_video = shape_video = mask_video = eye_video = mouth_centers = eyes_centers = audio_feature = 0
        else:
        ...
        for i in range(n_frames_total):
            ...
            if self.is_train:
                audio_feature_path = audio_feature_paths[start_idx + i]
                audio_feature_i = self.get_audio_feature(audio_feature_path)
  
            rgb_video = rgb_video_i if i == 0 else torch.cat([rgb_video, rgb_video_i], dim=0)
            if self.is_train:
                audio_feature = audio_feature_i if i == 0 else torch.cat([audio_feature, audio_feature_i], dim=0)
            # SHAPE
            if self.opt.use_shapes:
        ...
        if self.is_train:
            return_list = {'nmfc_video':nmfc_video, 'rgb_video':rgb_video, 'mask_video':mask_video, 'shape_video':shape_video,
                        ...
                        'change_seq':change_seq, 'A_paths':A_paths, "audio_feature": audio_feature}
        else:
            return_list = {'nmfc_video':nmfc_video, 'rgb_video':rgb_video, 'mask_video':mask_video, 'shape_video':shape_video,
                    'eye_video':eye_video, 'mouth_centers':mouth_centers, 'eyes_centers':eyes_centers,
    ...
    def get_audio_feature(self, audio_feature_path):
        return torch.from_numpy(np.load(audio_feature_path)).reshape(1, 512)
