...
from decoupled_contrastive_learning.model_render import DecoupledContrastiveLearning
from prompt_learning.supervision_model_render import EmotionSupervision
########################
#### Discriminators ####
########################
...
class Head2HeadModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # Image discriminator
        self.dcl_model = DecoupledContrastiveLearning()
        # self.emotion_model = EmotionSupervision()
        self.dcl_model.load_state_dict(torch.load("/data2/JM/code/NED-main/decoupled_contrastive_learning/model_ckpt_实验三/95_DCL.pth"))
        ...
        self.loss_names = ['G_VGG', 'G_GAN', 'G_GAN_Feat', 'D_real', 'D_fake', 'dcl', 'pc']
    ...
    def forward(self, tensors_list, mouth_centers=None, eyes_centers=None, audio=None):
        lambda_feat = self.opt.lambda_feat

        real_B, fake_B, real_A = tensors_list
        _, _, self.height, self.width = real_B.size()
        audio = audio.reshape(-1, 1, 512)
        loss_dcl, pc = self.dcl_model(self.resize(real_B), self.resize(fake_B), audio)
        #################### Losses ####################
        # VGG loss
        ...
        loss_list = [loss_G_VGG, loss_G_GAN, loss_G_GAN_Feat, loss_D_real, loss_D_fake, loss_dcl*2, pc]
...
class Head2HeadModelG(BaseModel):
    ...
    def forward(self, input_A, input_B, fake_B_prev, audio_feature):
        ...
        audio_feature_sub = audio_feature[:, self.n_frames_G-1:]
        fake_B_prev = fake_B_prev[:, -self.n_frames_G+1:].detach()
        return fake_B, real_A[:,self.n_frames_G-1:], real_B[:,self.n_frames_G-2:], fake_B_prev, audio_feature_sub
